...
!function ($) {
    ...
    BootstrapTable.prototype.mergeCellsById = function (options) {
        var row = options.index,
                rowId = options.id,
                col = $.inArray(options.field, this.getVisibleFields()),
                rowspan = options.rowspan || 1,
                colspan = options.colspan || 1,
                i, j,
                $trs = this.$body.find('>tr[data-uniqueid="' + rowId + '"]'),
                $td, $tr;
        ...
        $trs.each(function () {
            $tr = $(this);
            row = parseInt(this.getAttribute('data-index'));
            $td = $tr.find('>td').eq(col);

//            if (row < 0 || col < 0 || row >= this.$body.length) {
//                return;
//            }
